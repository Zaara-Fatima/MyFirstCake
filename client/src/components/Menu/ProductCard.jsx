...
export const ProductCard = ({className="", title="", price="", button, featured_img="", onAddtoCart}) => {
  ...
  return (
    <div className={`flex flex-col shadow-sm shadow-[#5a5a5a59] justify-center items-center overflow-hidden h-full w-full rounded-lg ${className}`}>
        ...
        <div>
        {button && <Button className="bg-[#b76e79] w-full md:w-auto shrink-0 text-white  " text="Add to Cart" icon='shopping_cart' onClick={onAddtoCart} />}
        </div>
        </div>
    </div>
  )
